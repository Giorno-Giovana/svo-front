<template>
  <corner-menu title="Центр уведомлений" corner="left">
    <corner-menu-element v-for="notification in notifications" :key="notification.id" @click="$emit('notificationChoose', notification)">
      <div class="flex align-middle justify-between w-full">
        <div class="flex">
          <div class="text-3xl text-compl-orange mr-4 mt-1">!</div>
          <div>
            <div class="text-lg">{{ notification.text }}</div>
            <div class="text-sm">Нажмите, чтобы создать задачу</div>
          </div>
        </div>
